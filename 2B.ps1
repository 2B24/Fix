<#
.SYNOPSIS
    Steam Manifest Fix (Official Filegear API Support)

.DESCRIPTION
    Finds the most recently updated installed Steam game, locates the matching stplug-in Lua file,
    parses depot IDs, queries SteamCMD API for manifest IDs, and downloads manifests.
    Uses the official Filegear API endpoint for backups.
...
# Global variable to store API Key for the session
$Global:ManifestApiKey = $null
...
function Try-DownloadUrl {
    ...
    for ($attempt = 1; $attempt -le $MaxRetries; $attempt++) {
        try {
            ...
            $response = Invoke-WebRequest -Uri $Url -Method Get -TimeoutSec 120 -OutFile $OutputFile -Headers $Headers -ErrorAction Stop
...
function Download-Manifest {
    ...
    # 1) Primary URL (GitHub)
    $githubUrl = "https://raw.githubusercontent.com/qwe213312/k25FCdfEOoEJ42S6/main/${DepotId}_${ManifestId}.manifest"

    Write-Host "  Trying GitHub: $githubUrl" -ForegroundColor DarkGray
    $githubResult = Try-DownloadUrl -Url $githubUrl -OutputFile $outputFile -MaxRetries 1 -Label "GitHub" -RetryDelaySeconds $RetryDelaySeconds
    ...
    # 2) Secondary (ManifestHub API)
    Write-WarningMsg "GitHub failed. Trying official ManifestHub API..."

    if ($null -eq $Global:ManifestApiKey -or $Global:ManifestApiKey -eq "") {
        Write-Host ""
        $Global:ManifestApiKey = Read-Host "  [?] ManifestHub API Key required. Please enter it"
        Write-Host ""
        if ([string]::IsNullOrWhiteSpace($Global:ManifestApiKey)) {
            Write-WarningMsg "No API key provided. Skipping backup."
            return @{
                Success  = $false
                Error    = "GitHub failed and no API key provided"
                Attempts = $githubResult.Attempts
            }
        }
    }

    $secondaryUrl = "https://api.manifesthub1.filegear-sg.me/manifest?apikey=$($Global:ManifestApiKey)&depotid=$DepotId&manifestid=$ManifestId"
    $secondaryLabel = "ManifestHub"

    # Some APIs accept key-in-URL only; some accept Bearer; sending both is usually harmless.
    $headers = @{
        "Authorization" = "Bearer $Global:ManifestApiKey"
    }

    Write-Host "  Trying backup API: $secondaryUrl" -ForegroundColor DarkGray
    $apiResult = Try-DownloadUrl -Url $secondaryUrl -OutputFile $outputFile -MaxRetries 2 -Label $secondaryLabel -RetryDelaySeconds $RetryDelaySeconds -Headers $headers
    ...
    return @{
        Success  = $false
        Error    = "GitHub failed (Status: $($githubResult.StatusCode)) AND ManifestHub API failed (Status: $($apiResult.StatusCode)): GitHub: $($githubResult.Error) | API: $($apiResult.Error)"
        Attempts = $githubResult.Attempts + $apiResult.Attempts
    }
}
...
if (-not $steamPath) { Write-ErrorMsg "Could not find Steam!"; exit 1 }
...
if (-not $libraries -or $libraries.Count -eq 0) { Write-ErrorMsg "No libraries found!"; exit 1 }
...
if (-not $selectedGame) { Write-ErrorMsg "No installed games found!"; exit 1 }
...
if (-not (Test-Path $luaPath)) { Write-ErrorMsg "Lua file not present!"; exit 1 }
Write-Success "Lua file found!"
Write-Host ""

Write-Status "Parsing Lua file for depot IDs..."
$depotIds = Get-DepotIdsFromLua -LuaPath $luaPath
if ($depotIds.Count -eq 0) { Write-ErrorMsg "No depot IDs found!"; exit 1 }
...
if (-not $appInfo -or $appInfo.status -ne "success") { Write-ErrorMsg "Failed to fetch app info!"; exit 1 }
...
foreach ($depotId in $depotIds) {
    ...
    if ($manifestId) { $downloadQueue += @{ DepotId = $depotId; ManifestId = $manifestId } }
}

if ($downloadQueue.Count -eq 0) { Write-WarningMsg "No manifests found!"; exit 1 }
Write-Success "Found $($downloadQueue.Count) depot(s) to download"
Write-Host ""

$depotCachePath = Join-Path $steamPath "depotcache"
if (-not (Test-Path $depotCachePath)) { New-Item -ItemType Directory -Path $depotCachePath -Force | Out-Null }
...
for ($i = 0; $i -lt $downloadQueue.Count; $i++) {
    ...
    if ($result.Success) {
        ...
        $sourceInfo = if ($result.Source) { " via $($result.Source)" } else { "" }
        Write-Success "Depot $depotId - Downloaded ($sizeStr)$sourceInfo"
    } else {
        $failedDepots += @{ DepotId = $depotId; ManifestId = $manifestId; Error = $result.Error }
        Write-ErrorMsg "Depot $depotId - Failed: $($result.Error)"
    }
}
